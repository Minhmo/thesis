...
BASE_FILE_NAME = 'GNN_Polynomial_random_so_results_'
...
for combination in combinations:
    ...
    for split in range(nDataSplits):
        ...
        SO = np.random.uniform(low=0., high=1., size=(nNodes, nNodes))
        ...
        if doPolynomialGNN:
            ...
            e, V = np.linalg.eig(SO)
            # \\\ Ordering
            highest_eig_val = np.max(np.diag(e)).real

            if highest_eig_val == 0:
                S, order = graphTools.permIdentity(SO)
            else:
                S, order = graphTools.permIdentity(SO / highest_eig_val)
