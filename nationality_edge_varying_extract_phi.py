...
writeVarValues(varsFile,
               {'authorName': 'gender_edge_varying',
                'nClasses': nClasses,
...
nShifts = 3  # K: number of shift taps
...
data = Utils.dataTools.AutorshipNationalityAmericanSO(ratioTrain, ratioValid, dataPath)
training_results = {}
...
for key in modelsGNN.keys():
    ...
    for thisModel in modelList:
        ...
        if thisModel in key:
            accBest[thisModel][0] = thisAccBest
...
result = {'nodes':  [int(x) for x in nodesToKeep], 'order':  [int(x) for x in order],
          'phi': phi.tolist()}


with open('EdgeVariGNN_nationality_american_phi.txt', 'w+') as outfile:
