...
BASE_FILE_NAME = 'GNN_Polynomial_unique_words_results_'
...
for auth in all_author_names:
    if path.exists("{0}{1}.txt".format(BASE_FILE_NAME, auth)):
        continue
...
word_count = dict(collections.Counter(all_selected_words).most_common()[::-1][:no_of_words_to_select])
labels, values = zip(*word_count.items())

nodes_to_keep = np.where(np.isin(data.functionWords, labels))[0]  # take the first dimension

assert nodes_to_keep.shape[0] == no_of_words_to_select

if doPrint:
    print('Popular words loaded: {0}'.format(function_words[nodes_to_keep]))
...
for combination in combinations:
    ...
    for split in range(nDataSplits):
        ...
        G = graphTools.Graph('fuseEdges', nNodes,
                             data.selectedAuthor['train']['WAN'][:, nodes_to_keep][:, :, nodes_to_keep],
                             'sum', graphNormalizationType, True,
                             False, False, nodesToKeep)
        ...
        if doPolynomialGNN:
            ...
            # \\\ Ordering
            S, order = graphTools.permIdentity(
                G.S / np.max(np.diag(G.E)))
