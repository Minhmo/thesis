...
BASE_FILE_NAME = 'GNN_Polynomial_phi_non_zero_results_'

PERCENTAGES = [0.01, 0.02, 0.05, 0.1, 0.2, 0.3, 0.5, 0.6]
...
with open('EdgeVariGNN_important_words_phi.txt', 'r') as f:
    file = json.load(f)
    phi_matrix = np.array(file[authorName]['phi'])
    nodes_to_keep = np.array(file[authorName]['nodes'])
    function_words = np.array(data.functionWords)
    phi_copy = np.copy(phi_matrix)
if doPrint:
    print("Loaded matrix PHI with shape: {0}, {1}".format(phi_matrix.shape[0], phi_matrix.shape[1]))

if phi_matrix.shape[0] < 5:
    print("Matrix PHI is too small: {0}, {1}".format(phi_matrix.shape[0], phi_matrix.shape[1]))
    exit(-1)
...
for percentage in PERCENTAGES:
    ...
    phi_matrix = phi_copy.copy()
    # zero out elements based on preffered percentage
    clause = np.argwhere(np.abs(phi_copy) > (1 - percentage) * np.max(np.abs(phi_copy)))

    x = list(list(zip(*clause))[0])
    y = list(list(zip(*clause))[1])

    if doPrint:
        print('No of edges: {}'.format(clause.shape[0]))

    mask = np.ones(phi_matrix.shape, dtype=np.bool)
    mask[x, y] = False

    phi_matrix[mask] = 0
    training_results[str(percentage)] = []
    ...
    training_results[str(percentage)] = {"acc": list(accBest['PolynomiGNN']), "non_zero": clause.shape[0]}
